...
Better Auth runs in Next.js with GitHub OAuth and authentication-only database access. Its JWT plugin signs ES256 tokens that the API validates against the web app's JWKS (ADR-015); project membership enforcement remains unimplemented. GitHub Apps repository integration is still pending. The dashboard remains a public preview with explicitly labeled sample requirements (ADR-018). Auth requires explicit secret, database URL, and canonical origin configuration; remote database connections verify TLS. Optional dashboard integration requires an environment key. See DEPLOYMENT.md.
...
Better Auth runs in Next.js with GitHub OAuth and authentication-only database access. Its JWT plugin is configured, and C# validates its tokens; project membership enforcement remains unimplemented. GitHub Apps repository integration is still pending. The dashboard remains a public preview with explicitly labeled sample requirements (ADR-018). Auth requires explicit secret, database URL, and canonical origin configuration; remote database connections verify TLS. Optional dashboard integration requires an environment key. Auth validates CA PEM and complete GitHub credential pairs before initialization. Newly written OAuth tokens use Better Auth encryption; historical plaintext is not automatically rewritten. See DEPLOYMENT.md.

Login and signup redirect an existing Better Auth session to the dashboard.
The route scaffold links its placeholder pages with example IDs until project
data and membership authorization are implemented.

The initial API exposes GET /health returning { "status": "ok" }. This is a liveness check, not database readiness. Development exposes /openapi/v1.json; production does not. EF Core maps six auth and four product tables in public, and owns InitialSchema and future migrations. Startup does not create, migrate, or query the database. Database use fails explicitly if ConnectionStrings:Database is missing. RLS and revoked browser-role grants deny Data API access by default; this does not replace server-side project authorization. Supabase-managed schemas must not be modified. See DATABASE.md.
