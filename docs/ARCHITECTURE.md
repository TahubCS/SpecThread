...
Better Auth runs in Next.js with GitHub OAuth and authentication-only database access. Its JWT plugin is configured, and C# validates its tokens; project membership enforcement remains unimplemented. GitHub Apps repository integration is still pending. The dashboard remains a public, empty preview. Auth requires explicit secret, database URL, and canonical origin configuration; remote database connections verify TLS. Optional dashboard integration requires an environment key. Auth validates CA PEM and complete GitHub credential pairs before initialization. Newly written OAuth tokens use Better Auth encryption; historical plaintext is not automatically rewritten. See DEPLOYMENT.md.
