...
Better Auth runs in Next.js with authentication-only database access. Users sign in with email and password (verified email required), Google, or GitHub, and link or unlink Google and GitHub from /account (ADR-016). Verification and reset emails are sent through Resend. Its JWT plugin is configured, but C# token validation and project membership enforcement remain unimplemented. GitHub Apps repository integration is still pending. The dashboard remains a public, empty preview. Auth requires explicit secret, database URL, and canonical origin configuration; remote database connections verify TLS. Optional dashboard integration requires an environment key. See DEPLOYMENT.md.
...
Better Auth runs in Next.js with GitHub OAuth and authentication-only database access. Its JWT plugin is configured, but C# token validation and project membership enforcement remain unimplemented. GitHub Apps repository integration is still pending. The dashboard remains a public, empty preview. Auth requires explicit secret, database URL, and canonical origin configuration; remote database connections verify TLS. Optional dashboard integration requires an environment key. Auth validates CA PEM and complete GitHub credential pairs before initialization. Newly written OAuth tokens use Better Auth encryption; historical plaintext is not automatically rewritten. See DEPLOYMENT.md.
