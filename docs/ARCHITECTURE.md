...
Better Auth runs in Next.js with authentication-only database access. Users sign in with email and password (verified email required), Google, or GitHub, and link or unlink Google and GitHub from /account (ADR-016). Verification and reset emails are sent through Resend. Its JWT plugin signs ES256 tokens that the API validates against the web app's JWKS (ADR-015); project membership enforcement remains unimplemented. GitHub Apps repository integration is still pending. The dashboard remains a public, empty preview. Auth requires explicit secret, database URL, canonical origin, and email delivery configuration; remote database connections verify TLS. Optional dashboard integration requires an environment key. Auth validates CA PEM and complete GitHub credential pairs before initialization. Newly written OAuth tokens use Better Auth encryption; historical plaintext is not automatically rewritten. See DEPLOYMENT.md.
