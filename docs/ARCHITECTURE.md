...
Better Auth runs in Next.js with GitHub OAuth and authentication-only database access. Its JWT plugin is configured, but C# token validation and project membership enforcement remain unimplemented. GitHub Apps repository integration is still pending. The dashboard remains a public, empty preview. Auth requires explicit secret, database URL, and canonical origin configuration; remote database connections verify TLS. Optional dashboard integration requires an environment key. See DEPLOYMENT.md.
