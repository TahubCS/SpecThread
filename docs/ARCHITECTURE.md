...
Better Auth runs in Next.js with authentication-only database access. Users sign in with verified email and password, Google, or GitHub, and manage provider links at /account (ADR-016). Verification and reset emails use Resend. The JWT plugin signs ES256 tokens that the API validates against the web app's JWKS (ADR-015). Project membership enforcement and GitHub Apps repository integration remain pending. The dashboard is a public preview with labeled sample requirements (ADR-020). Auth requires an explicit secret, database URL, canonical origin, and email delivery configuration; remote database connections verify TLS. Optional dashboard integration requires an environment key. Auth validates CA PEM and complete GitHub credential pairs before initialization. Newly written OAuth tokens use Better Auth encryption; historical plaintext is not automatically rewritten. See DEPLOYMENT.md.

Login and signup redirect an existing Better Auth session to the dashboard. The route scaffold links placeholder pages with example IDs until product data and membership authorization are implemented.

The API exposes anonymous GET /health returning { "status": "ok" } and authenticated GET /me returning { "userId": "<sub>" }. Every other endpoint requires a valid Better Auth JWT by default (ADR-015). /health is a liveness check, not database readiness. Development exposes /openapi/v1.json; production does not. EF Core maps six auth tables (user, session, account, verification, jwks, rateLimit) and four product tables in public, and owns the InitialSchema and AuthRateLimits migrations and future migrations. Startup does not create, migrate, or query the database. Database use fails explicitly if ConnectionStrings:Database is missing. RLS and revoked browser-role grants deny Data API access by default; this does not replace server-side project authorization. Supabase-managed schemas must not be modified. See DATABASE.md.
